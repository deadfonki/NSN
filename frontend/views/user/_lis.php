...
<div class="search-item col-md-8 col-sm-12" style="border: 1px solid gray;margin-top: 2%">
    <div class="img-responsive col-md-2 col-sm-2">
        ...
        $request = \frontend\models\Requests::findAll(['sender_id' => Yii::$app->user->id,'receiver_id' => $model->id]);
        $request_1 = \frontend\models\Requests::findAll(['sender_id' => $model->id,'receiver_id' => Yii::$app->user->id]);
        $friend = \frontend\models\Friends::findAll(['user_1' => Yii::$app->user->id,'user_2' => $model->id]);
        $r1 = count($request_1);
        $r = count($request);
        for ($i = 0;$i<$r1;$i++)
        {
        ...
        {
            echo  $img = '<a href="/user/show?id'.$model->id.'"><img class="img-circle " src="/images/'.$model->username.'/avatar.png"></a>';
        }
        else{
            echo  $img = '<a href="/user/show?id'.$model->id.'"><img class="img-circle " src="/images/default_avatar.png"></a>';
        }
        ?>
    </div>
